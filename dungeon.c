...
// typedef struct {
//     char map[21][80];  // The dungeon layout
//     int hardness[21][80];     // Hardness values
//     int playerX, playerY;     // PC's position
// } Dungeon;
...
void saveDungeon(char dungeonMap[21][80], int hardness[21][80], int totalRooms, int numUpStairs, int numDownStairs)
{
    ...
    char filename[256];
    snprintf(filename, sizeof(filename), "%s/dungeon", dirpath);
